...
public class TestBase {
    ...
    public void setUp() {
        ...
        capabilities.setCapability("videoName", "FiestaPizzaTestCaseForFirstSendOrder.mp4");
        capabilities.setCapability("name", "FiestaPizzaTestCaseForFirstSendOrder");
    ...
    public void openURL()
    {
        open("https://fiesta-pizza.ru/");
    }

    @Step("Выбираю рандомный город. Выбор городов реализован через Drop Down")

    public void restSelect() {
        SelenideElement modalContent = $x("//div[@class = \"modal-arora-body\"]");
        modalContent.shouldBe(visible);
        List<SelenideElement> terminalSwitch = elements(By.xpath("//div[@class = \"modal-arora-body\"]//a[contains(@class , \"city-select-link\") and not (contains(text(), \"Стерлитамак\"))and not (contains(text(), \"Казань\"))]"));
...
/*        $x("//span[@class = \"close\"]").shouldBe(visible).click();*/
    ...
    public void mathRandomHead() {
        List<SelenideElement> mathRandomHead = elements(By.xpath("//div[@class = \"cat-list\"]/ul/li/a[contains(text(), \"\") and not (contains(text(), \"Пицца\"))and not (contains(text(), \"Акции\"))]"));
    ...
    public void pickRandCards() {
        List<SelenideElement> clickRandomCards = elements(By.xpath("//div[contains(@class, \"productBox\") and not (contains(@class, \"action-wrapper\"))]//div[@class = \"summary\"]//a[contains(@class, \"add-to-basket\") and not (contains(@class, \"d-none\"))]"));
